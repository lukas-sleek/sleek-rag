...
def event_text(event: dict) -> str:
    """Concatenate text parts. Defensively still filters out any `thought`
    parts in case an upstream config flips back to include_thoughts=True —
    user-facing streamed text must never carry chain-of-thought."""
    return "".join(
        p.get("text") or ""
