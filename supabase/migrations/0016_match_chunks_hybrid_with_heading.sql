-- Migration 0016: expose heading_path + chunk_index from match_chunks_hybrid.
--
-- Plan 17.4 T2b: the rater needs each chunk's heading path (so it can spot
-- "all chunks are sub-rows of the same table; no headline row in the set")
-- and chunk_index (for stable in-document ordering when multiple chunks of
-- the same section come back). Both columns already exist on
-- `document_chunks` — we just need to surface them through the RPC.
--
-- New columns are appended to the end of RETURNS TABLE so the existing
-- `_rpc_row_to_chunk` mapping keeps working unchanged (it only reads named
-- keys). The function body is otherwise identical to migration 0014.
--
-- Postgres rejects `create or replace` when the RETURNS TABLE shape changes,
-- so we drop + recreate. Same signature on the input side (project_id,
-- embedding, query, …) so callers don't need updating.

drop function if exists public.match_chunks_hybrid(
  uuid, vector, text, int, uuid[], text, int, text, text, int
);
